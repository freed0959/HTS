...
/****** Object:  StoredProcedure [dbo].[SP_Upd_trCount]    Script Date: 09/29/2014 12:31:47 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_Upd_trCount]
	(	@trCount as int
		, @idpl as bigint= NULL
...
BEGIN
	...
	update tbl_ActMasterBlend set TruckCount = @trCount 
	...
	BEGIN
		...
		WHERE (
			(pb.pln_date BETWEEN DATEADD(HOUR, 0, convert(nvarchar, @plan_hour, 112)) 
				AND DATEADD(HOUR, 6, convert(nvarchar, pb.pln_date, 112)))
